...
You are welcome to
<a href="http://www.almanac.bc.ca/cgi-bin/guest-book-sign-in.pl">add
your own</a>.
...
sub createGuestBook
{
	open(GUESTBOOK, ">$guestBookDir$guestBookFilename");
	#
	# Remember, if there's an error opening a file, then a print
	# to that filehandle is simply a no-op.  Errors are handled
	# elsewhere.
	#
	print $emptyGuestBookText;
	close(GUESTBOOK);
...
{
	open(SPECIALGUESTBOOK, ">$guestBookPrivateDir$specialGuestBookFilename");
	#
	# Remember, if there's an error opening a file, then a print
	# to that filehandle is simply a no-op.  Errors are handled
	# elsewhere.
	#
	print $emptySpecialGuestBookText;
